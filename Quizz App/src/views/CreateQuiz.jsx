  ...
  return (
    <>
          ...
          <Popover open={open} onOpenChange={setOpen}>
            <PopoverTrigger asChild>
              ...
              >
                {timeLimit
                  ? timeRanges.find(
                      (framework) => framework.value === timeLimit
                    )?.label
                  : "Set time limit"}
      ...
      {showTeams && filteredTeams.length > 0 && (
        <div>
          <h1>
            TUK SE POKAZVAT OTBORITE V KOITO UCHSTVA SLED KATO E NATISNAL Assign
            to group{" "}
          </h1>
          {filteredTeams.map((team) => (
            <div key={team.id}>
              <p>{team.name}</p>
              <button onClick={() => handleAddQuizToTeam(team.id)}>
                Add quiz to team
              </button>
            </div>
          ))}
      ...
      {showUsers && students.length > 0 && (
        <div>
          <input
            className="input input-bordered w-24 md:w-auto mt-2 mb-2"
            type="text"
            value={searchTerm}
              ...
              <div className="p-3">
                <div className="flex flex-col items-start justify-start w-800px ">
                  ...
                  {answers.map((answer, index) => (
                    <div
                      key={index}
                      className="flex flex-row items-start justify-start w-600px "
                    >
                      <Input
                        type="text"
      ...
      <div className="flex flex-col items-center justify-center">
        ...
        <input
          type="number"
          value={grades.good}
          onChange={(e) => setGrades({ ...grades, good: e.target.value })}
          placeholder="Satisfactory/Good border"
        />
        <input
          type="number"
          value={grades.bad}
          onChange={(e) => setGrades({ ...grades, bad: e.target.value })}
          placeholder="Satisfactory/Bad border"
        />
        <p>Good: {grades.good} and above</p>
        <p>
          Satisfactory: {grades.bad} - {grades.good}
        </p>
