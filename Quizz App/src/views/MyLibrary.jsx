...
const MyLibrary = () => {
  ...
  useEffect(() => {
    ...
    if (
      userData &&
      (userData.role === "teacher" || userData.isAdmin === true)
    ) {
      unsubscribeQuizByCreator = getQuizByCreator(
        userData.username,
        (quizzes) => setMyQuizzes(quizzes)
      );
    }
    const unsubscribeUserQuizzes = getUserQuizzes(
      userData?.username,
      async (quizzes) => {
        const quizzesArray = await Promise.all(
          Object.entries(quizzes).map(async ([id, quiz]) => {
        ...
        const nonCompletedQuizzes = quizzesArray.filter(
          (quiz) => !quiz.isCompleted
        );
        setStudentQuizzes({
          completed: completedQuizzes,
          nonCompleted: nonCompletedQuizzes,
        });
      }
    );
  ...
  const quizCreation = async () => {
    ...
    try {
      let category =
        quiz.category.trim() !== "" ? quiz.category : selectedOption;
        ...
        <div className="flex flex-col h-full items-start justify-start p-6">
          ...
          <Dialog onClose={handleCloseDialog}>
            ...
            <DialogContent className="sm:max-w-[425px] bg-white dark:bg-neutral text-black dark:text-white">
              ...
              <DialogFooter>
                <Button type="submit" onClick={quizCreation}>
                  Create Quiz
                </Button>
              </DialogFooter>
            </DialogContent>
